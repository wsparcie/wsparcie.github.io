...
class TranslationManager {
  ...
  createTranslationSwitchers() {
    ...
    if (document.querySelectorAll('.translation-switcher').length === 0) {
  ...
  setLanguage(lang) {
    ...
    allTranslationButtons.forEach((button) => this.updateButtonText(button))
    ...
    elements.forEach((element) => {
      ...
      if (translations[lang] && translations[lang][key]) {
        if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
          ...
          if (parentLi) {
            const strongElement = parentLi.querySelector('strong')
            if (strongElement) {
              const newText = translations[lang][key]
              // Replace only the template content, keep the strong element
  ...
  handleTemplateElements(lang) {
    // Find all li elements with templates
    const listItems = document.querySelectorAll('li')

    listItems.forEach((li) => {
      const template = li.querySelector('template[data-translate]')
      const strong = li.querySelector('strong[data-translate]')

      if (template && strong) {
        const templateKey = template.getAttribute('data-translate')
        const strongKey = strong.getAttribute('data-translate')

        if (translations[lang] && translations[lang][templateKey]) {
          // Clear the li content and rebuild it
          const strongText = translations[lang][strongKey] || strong.textContent
          const templateText = translations[lang][templateKey]

          li.innerHTML = `<strong data-translate="${strongKey}">${strongText}</strong> ${templateText}`
        }
      }
    })
  }
